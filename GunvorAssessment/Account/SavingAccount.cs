...
namespace GunvorAssessment.Account
{
    public class SavingAccount : IAccount
    {
        ...
        public SavingAccount(int accountNumber, IDateTimeService dateTimeService, ILockDownManager lockDownManager, ITransactionAudit transactionAudit)
        {
        ...
        public async Task WithdrawAsync(decimal amount)
        {
            try
            {
                ...
                var temp = Balance * (decimal)0.1;
                if (amount > 0 && Balance >= amount && amount <= temp)
                {
                    var tempValue = Balance;
                    var newValue = tempValue - amount;
                    Balance = newValue;
